...
class AdminMDictionary extends AdminPanel 
{
    ...
    public function actionLanguages($param)
    {
        $this->Tag('languages', $this->_table->GetAll(true, false, array('name')));
    }
  
    public function actionDeleteSiteLanguage($param)
    {
        $this->_table->current = $param->key;
        if($this->_table->id != $param->key) {
            return $this->_Referer();
        }
        if($this->_table->active == '1' && $this->_IsOneActiveLanguageLeft()) {
            return;
        }
        if(parent::actionDelete($param) == 0) {
            fsCache::Clear(FunctionsLanguages::$CACHE_PREFIX);
        }
    }
    
    private function _IsOneActiveLanguageLeft() 
    {
        $count = $this->_table->GetCount('`active` = "1"');
        if($count <= 1) {
            $this->_Referer();
            $this->Message(T('XMLcms_cannot_delete_last'));
            return true;
        }
        return false;
    }
    ...
    {
        fsCache::Clear(FunctionsLanguages::$CACHE_PREFIX);
        parent::actionActivate($param);
    }
    
    public function actionDeActivate($param)
    {
        if($this->_IsOneActiveLanguageLeft()) {
            return;
        }
        fsCache::Clear(FunctionsLanguages::$CACHE_PREFIX);
        parent::actionDeActivate($param);
    }
    
    private function _CheckSiteLanguage($param)
    {
        $this->_Referer();
        if($param->name === '' || ($param->Exists('key') && $param->key == '')) {
    ...
    public function actionDoAddSiteLanguage($param)
    {
        if(!$this->_CheckSiteLanguage($param) || !$this->_CheckUnique($param->name, 'name')) {
            return;
        }
        if($this->_table->Add($param->name) > 0) {
            return $this->Message(T('XMLcms_added'));
        } 
        fsCache::Clear(FunctionsLanguages::$CACHE_PREFIX);
        $this->Message(T('XMLcms_error_action'));
    }
    
    public function actionDoEditSiteLanguage($param)
    {
        if(!$this->_CheckSiteLanguage($param) 
            || !$this->_CheckUnique($param->name, 'name', $param->key, 'name')) {
            return;
        }
        if($this->_table->Edit($param->key, $param->name)) {
